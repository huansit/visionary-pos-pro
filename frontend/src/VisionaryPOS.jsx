...
function useKopokopoLookup(branchId, codeLast4) {
  const normalizedCode = normalizeMpesaCodeLast4(codeLast4);
  const lookupCode = normalizedCode.length === 4 ? normalizedCode : "";
  const [state, setState] = useState({ enabled: null, providerRequired: null, loading: false, transactions: [], error: "" });
  useEffect(() => {
    let active = true;
    if (!branchId) {
      setState({ enabled: null, providerRequired: null, loading: false, transactions: [], error: "" });
      return () => { active = false; };
    }
    setState((current) => ({ ...current, loading: Boolean(lookupCode), transactions: [], error: "" }));
    const timer = window.setTimeout(async () => {
      try {
        const result = await lookupKopokopoTransactions(branchId, lookupCode);
        if (active) setState({
          enabled: result.enabled !== false,
          providerRequired: result.providerRequired === true,
          loading: false,
        ...
        if (active) setState((current) => ({
          ...
          loading: false,
          transactions: [],
          error: current.providerRequired === false
            ? "Kopo Kopo could not be reached. Manual entry is still available for this branch."
            : "Kopo Kopo verification is unavailable. Try again before recording an M-Pesa payment.",
        }));
      }
    }, lookupCode ? 250 : 0);
    return () => {
...
.mpesa-ledger-desktop .mpesa-allocation-menu[open]{z-index:5}
.mpesa-ledger-desktop .mpesa-allocation-menu .mpesa-allocations{position:absolute;top:100%;left:0;z-index:6;width:max-content;min-width:100%;max-width:min(360px,70vw);box-shadow:0 12px 28px rgba(0,0,0,.28)}
...
.mpesa-receipt-status span{display:block;color:var(--muted-2);font-size:9px;font-weight:750;text-transform:uppercase}
...
.kopokopo-verified{display:flex;align-items:flex-start;gap:8px;color:var(--ok)}
.kopokopo-verified svg{width:16px;height:16px;flex:0 0 auto}
.kopokopo-verified>div{display:grid;gap:2px}
.kopokopo-verified b{font-size:12px}
.kopokopo-verified span{color:var(--muted);font-size:11px}
.mpesa-receipt-status .provider-detail{grid-column:span 2}
...
.settlement-inline-total small{color:var(--muted-2);font-size:10.5px}
...
function BulkSettleDayModal({ invoices, activeCashierNames = [], initialCashier = "all", branch, data, update, cur, user, onClose }) {
  ...
  else if (mpesaCents > 0 && providerLookup.providerRequired !== false && !verifiedReceipt) validationError = "This branch requires a verified Kopo Kopo transaction. Manual M-Pesa amounts are disabled.";
  ...
  const changeReceiptAmount = (value) => {
    const clean = value.replace(/[^\d.]/g, "");
    const paidCents = centsFromInput(clean);
    setMpesaReceiptAmount(clean);
    setMpesaAmount(moneyInputValue(Math.min(totalCents, paidCents)));
    setError("");
  };
  ...
  const settleInvoices = async () => {
    ...
    const invoicePaidCents = new Map();
    const receipt = mpesaCents > 0 ? (existingReceipt || {
      id: uid("mpesa-receipt"),
      branchId: branch.id,
      codeLast4: normalizedMpesaCode,
      totalCents: receiptTotalCents,
      allocatedCents: 0,
      remainingCents: receiptTotalCents,
      registeredAt: ts,
      registeredByName: actorName,
    }) : null;
      ...
      <div className="modal settlement-modal" onClick={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            ...
            <div className="title" style={{ fontSize: 20 }}>Apply invoice payment</div>
          </div>
        ...
        <div className="grid2 mpesa-receipt-grid" style={{ marginTop: 14 }}>
          <div>
            <label className="label">Last 4 of M-Pesa code</label>
            <input className="input innum" inputMode="text" maxLength={4} value={mpesaCode}
              onChange={(event) => changeMpesaCode(event.target.value)} placeholder="e.g. 7X9Q" />
          </div>
          {providerLookup.providerRequired === false ? <div>
          ...
          <div>
            <label className="label">M-Pesa to apply now</label>
        ...
        <div className="grid2" style={{ marginTop: 16 }}>
          ...
          <button className="btn btn-primary" onClick={settleInvoices}
            ...
            <Check /> {submitting ? "Recording..." : `Apply ${fmt(settlementCents, cur)}`}
          </button>
        </div>
      </div>
    </div>
        ...
        {!voidPending && !voidApproved && out > 0 ? (
          <div className="settlement-box invoice-payment-panel">
            ...
            <div className="grid2 mpesa-receipt-grid invoice-split-payment">
              ...
              {providerLookup.providerRequired === false ? <label><span>Total paid on code - manual</span>
                <input className="input" inputMode="decimal" value={mpesaReceiptAmount} disabled={Boolean(existingReceipt)}
                  onChange={(e) => {
