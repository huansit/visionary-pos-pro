...
const PRODUCT_IMAGE_FALLBACK_BY_SKU: Record<string, string> = {
  // Exact uploads for these SKUs are not present in Cloudinary yet. These are
...
const PRODUCT_IMAGE_LABEL_BY_SKU: Record<string, string> = {
  "SIP0113": "V&A WINE 750ML",
  "SIP0165": "REMY MARTIN VSOP 750ML",
};

function generatedProductImageUrl(product: { sku?: string; barcode?: string; } | null | undefined): string {
...
export function knownProductImageUrl(product: { sku?: string; barcode?: string; } | null | undefined): string {
  const sku = String(product?.sku || product?.barcode || '').trim().toUpperCase();
  const id = sku ? PRODUCT_IMAGE_BY_SKU[sku] || PRODUCT_IMAGE_FALLBACK_BY_SKU[sku] : '';
  return id ? PRODUCT_IMAGE_BASE + id : generatedProductImageUrl(product);
}

export function productDisplayImage(product: { sku?: string; barcode?: string; image?: string; imageUrl?: string; } | null | undefined): string {
  const known = knownProductImageUrl(product);
  const sku = String(product?.sku || '').trim().toUpperCase();
  if (known) return known;
  if (/^SIP\d{4}$/.test(sku)) return '';
