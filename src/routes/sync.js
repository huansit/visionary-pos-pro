...
import { pool, q, serverNow } from "../db.js";
...
router.post("/push", async (req, res) => {
  ...
  try {
    ...
    await client.query("UPDATE devices SET last_seen_at = now() WHERE device_id = $1", [req.deviceId]);
...
async function insertAppendOnlyEvent(client, ev, type, deviceId, ts) {
  ...
  if (inserted.rows[0]) return inserted.rows[0].server_ts;
...
async function upsertMutableRecord(client, ev, type, deviceId, ts) {
  ...
  if (result.rows[0]) return result.rows[0].server_ts;
